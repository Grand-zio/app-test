...
def main():
    ...
    try:
        ...
        for i in range(len(xml_report_list)):
            cmd = f"cmd.exe /c allure generate {xml_report_list[i]} -o {html_report_list[i]}"
        ...
        # for templates in show_name_list:
        #     SendEmail().Send_Email(html_zip_path, zip_file_name, templates)
        # log.info("-----邮件发送完成------")
